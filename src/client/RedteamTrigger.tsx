...
import type { RedteamTriggerProps } from './contract/slots.ts'
 ...
 */
export function RedteamTrigger({ wide, t, useStore, actions }: RedteamTriggerProps) {
  const open = useStore(s => s.open)
  return (
    <button
      type="button"
      className={clsx(css.trigger, wide ? css.wide : css.rail)}
      aria-pressed={open}
      aria-label={t('trigger.aria')}
      onClick={actions.open}
    >
      <IconWarningOutline16 size={16} className={css.icon} />
