...
const TARGET_KINDS = new Set(['targets.kind.host', 'targets.kind.domain', 'targets.kind.web', 'targets.kind.cloud']);
const TARGET_STATES = new Set(['targets.state.queued', 'targets.state.recon', 'targets.state.breached', 'targets.state.dropped']);
const TARGET_RIGHTS = new Set(['targets.rights.none', 'targets.rights.user', 'targets.rights.admin', 'targets.rights.system', 'targets.rights.domain-admin']);
/**
    ...
    ctx.effect(() => host.webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                let suffix = 2;
                while (loaded.dataset.targets.some(row => row.id === id))
                    id = `manual-${address}-${suffix++}`;
                const target = {
                    id,
                    address,
                ...
                const next = { ...loaded.dataset, targets: [...loaded.dataset.targets, target] };
                const files = [];
                if (loaded.sourcePath !== undefined) {
