...
export function parseNmapXml(text) {
    ...
    for (const hostMatch of text.matchAll(/<host[\s>][\s\S]*?<\/host>/g)) {
        ...
        for (const portMatch of host.matchAll(/<port\s+protocol="(\w+)"\s+portid="(\d+)"[^>]*>([\s\S]*?)<\/port>/g)) {
            if (/<state\s+state="open"/.test(portMatch[3] ?? ''))
                ports.push(`${portMatch[2] ?? ''}/${portMatch[1] ?? ''}`);
        }
...
export function parseNucleiJsonl(text) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (templateId === '' || host === '' || seen.has(key))
            continue;
        seen.add(key);
 ...
 */
export function parseBurpIssues(text) {
    let value;
    try {
        value = JSON.parse(stripBom(text));
    }
